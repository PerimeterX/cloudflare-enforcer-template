...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const { contextId, request, response } = await env.HUMAN_ENFORCER_SERVICE.enforce(req);
    ...
    return await env.HUMAN_ENFORCER_SERVICE.postEnforce(contextId, originResponse);
  },
}; 